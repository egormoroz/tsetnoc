...
from app.core.models import User, Problem, PendingSub, Contest
from app.common.errors import ErrorCode, MalformedError

from .schemas import ContestDTO, GetProblemDTO, NewProblemDTO, SubProcessResultDTO, NewSubmissionDTO
...
@app.post("/contest/{cid}/join")
async def join_contest(uid: int, cid: int, conts: Contests):
    try:
        await conts.add_participants(cid, [uid])
        logger.info(f"user(id={uid}) joined contest(id={cid})")
        return {"message": "joined contest successfully"}
    except MalformedError as e:
        if e.ec == ErrorCode.FOREIGN_KEY_ERROR:
            logger.error(f"user(id={uid}) failed to joined contest(id={cid}): ",
                         "either user or contest doesn't exist")
            detail ="user/contest doesn't exist"
        elif e.ec == ErrorCode.ALREADY_EXISTS:
            logger.error(f"user(id={uid}) already joined contest(id={cid}) again")
            detail = "user already joined the contest"
        else:
            logger.error("unknown error on add_participant")
            detail = "unknown error"
        raise HTTPException(status_code=422, detail=detail)


@app.post("/contest/create")
async def create_contest(cont_dto: ContestDTO, conts: Contests):
    contest = Contest(id=0, name=cont_dto.name, 
                      start=cont_dto.start, end=cont_dto.end)
    await conts.add(contest)
    logger.info(f"created {contest=}")
    return {"message": "contest created successfully", "id": contest.id}
    ...
    try:
        ...
        if e.ec == ErrorCode.FOREIGN_KEY_ERROR:
            logger.error(f"failed to add problems to contest(id={cid}): ",
                         "the contest/one of problems doesn't exist")
            detail ="contest/one of problems doesn't exist"
        elif e.ec == ErrorCode.ALREADY_EXISTS:
            logger.error("one of the problems is already present ",
                         f"in contest(id={cid})")
            detail = "one of the problems already present"
        else:
            logger.error("unknown error on contests.add_problem")
            detail = "unknown error"
        raise HTTPException(status_code=422, detail=detail)
...
async def get_contest_problems(
    ...
    if not await users.joined_contest(uid, cid):
        logger.error(f"user(id={uid}) tried to sneak a peek",
                     f"contest(id={cid}) problem list")
    ...
    if fmt == ProbListFmt.IDS:
        return await probs.get_ids_by_contest(cid)
    elif fmt == ProbListFmt.IDS_NAMES:
        return [{"id": p.id, "name": p.name}
                for p in await probs.get_by_contest(cid)]
    else:
...
async def get_problem(uid: int, problem_id: int, probs: Problems, users: Users):
    ...
    prob = await probs.get(problem_id)
...
async def get_submission_list(
    ...
    fmt: SubListFmt,
    subs: Submissions,
    by_problem: int|None = None,
    by_contest: int|None = None,
):
